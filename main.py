import asyncio
from crawl4ai import AsyncWebCrawler, PruningContentFilter, DefaultMarkdownGenerator, CrawlerRunConfig
...
def extract_news_urls(markdown_content: str, base_domain: str, current_page_url: str) -> set[str]:
    """
    ...
    logging.debug(f"Found {len(all_potential_strings)} potential URL strings in markdown from {current_page_url}")
    ...
    for url_candidate_str in all_potential_strings:
        ...
        if not ('newsdetail.aspx' in url_candidate_str.lower() and 'newsid=' in url_candidate_str.lower()):
            continue
        ...
        if not parsed_temp_url.scheme:
            if parsed_temp_url.netloc: # If it looks like a domain is present
                 ...
                 absolute_url = urljoin(current_page_url, absolute_url) # Re-join if needed
                 parsed_temp_url = urlparse(absolute_url)
        ...
        if (parsed_new_url.netloc and 
            parsed_new_url.netloc.lower() == base_domain.lower() and
            '/newsdetail.aspx' in parsed_new_url.path.lower() and
            'newsid=' in parsed_new_url.query.lower()):
            
            # Check if newsID is numeric
            query_params = parse_qs(parsed_new_url.query)
            news_id_values = query_params.get('newsID', []) # parse_qs returns list of values
            if news_id_values and news_id_values[0].isdigit():
                 extracted_urls.add(absolute_url)
                 logging.debug(f"EXTRACTED valid news URL: {absolute_url} (from: {url_candidate_str})")
            else:
                logging.debug(f"REJECTED candidate (non-numeric newsID): {absolute_url} (from: {url_candidate_str})")
        else:
            logging.debug(f"REJECTED candidate (domain/path/query mismatch): {absolute_url} (from: {url_candidate_str})")
...
async def crawl_and_extract_content(
    ...
    if not current_domain or current_domain.lower() != target_domain.lower():
        logging.warning(f"Skipping URL {url} as its domain '{current_domain}' does not match target '{target_domain}'")
        return None
    ...
    if markdown_file_path.exists():
        logging.info(f"Markdown already exists for {url} at {markdown_file_path}, reading from disk.")
        try:
    ...
    try:
        ...
        if result.success and result.markdown:
            ...
            logging.error(f"Failed to crawl or get markdown for {url}. Error: {result.error if result else 'Unknown error'}")
            return None
    except Exception as e:
...
async def main(start_url: str):
    ...
    if not base_domain:
        logging.error(f"Could not determine base domain from initial URL: {start_url}. Exiting.")
        return

    logging.info(f"Starting crawl with initial URL: {start_url} (Base Domain: {base_domain})")
